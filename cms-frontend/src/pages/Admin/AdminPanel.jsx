...
export default function AdminPanel() {
  ...
  const handleSavePurpose = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await api.put(`/admin/purposes/${editingId}`, { name: newPurposeName, description: newPurposeDesc });
        toast.success("Purpose Successfully Updated!");
      } else {
        await api.post('/admin/purposes', { name: newPurposeName, description: newPurposeDesc });
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10">
      ...
      <div className="flex border-b mb-6">
        <button 
          className={`px-6 py-3 font-medium text-sm transition-colors ${activeTab === 'audit' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
          onClick={() => { setActiveTab('audit'); setConfigView('hub'); setSearchTerm(''); }}
        >
          Compliance & Audit Logs
        </button>
        <button 
          className={`px-6 py-3 font-medium text-sm transition-colors ${activeTab === 'config' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
        ...
        <div className="bg-white rounded-xl shadow border border-gray-200 p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {isLoading ? (
                  <tr><td colSpan="7" className="text-center py-8 text-gray-500 font-medium">Loading secure logs...</td></tr>
                ) : filteredLogs.length === 0 ? (
                  <tr><td colSpan="7" className="text-center py-8 text-gray-500">No logs found matching your criteria.</td></tr>
                ) : (
                  filteredLogs.map(log => (
                    <tr key={log.id} className="border-b border-gray-100 hover:bg-slate-50 transition font-mono text-xs text-gray-600">
                      <td className="py-3 px-4 whitespace-nowrap">{new Date(log.timestamp).toLocaleString()}</td>
                      <td className="py-3 px-4">
                        <span className={`px-2 py-1 rounded font-bold ${log.actionType === 'GRANT' ? 'bg-green-100 text-green-800' : log.actionType === 'WITHDRAW' ? 'bg-red-100 text-red-800' : log.actionType === 'VALIDATE' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100'}`}>
                          {log.actionType}
                        </span>
                      </td>
